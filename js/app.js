...
class App {
    ...
    async processNotificationQueue() {
        ...
        el.innerHTML = `<div style="width:32px;height:32px;border-radius:12px;background:rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;"><i class="fas ${icon}"></i></div><div style="flex:1"><div style="font-weight:700;font-size:0.9rem">${title}</div><div style="font-size:0.75rem">${message}</div></div>`;
    ...
    async buyPlan(type) {
        ...
        const ad = await this.showAd('AdBlock1');
    ...
    async exchange(amount) {
        ...
        const ad = await this.showAd('AdBlock2');
    ...
    async loadTasks() {
        const main = [
            { id: 't1', name: 'Join Telegram', reward: 5, url: 'https://t.me/STARZ_NEW', img: APP_CONFIG.BOT_AVATAR },
            { id: 't2', name: 'Follow Twitter', reward: 5, url: 'https://twitter.com', img: APP_CONFIG.BOT_AVATAR },
            { id: 't3', name: 'Subscribe YouTube', reward: 10, url: 'https://youtube.com', img: APP_CONFIG.BOT_AVATAR }
        ];
        const partner = [
    ...
    renderTeam() {
        ...
        el.innerHTML = `
            ...
            <div class="stats-grid"><div class="stat-mini"><i class="fas fa-users"></i><span class="stat-num">${this.userState?.totalReferrals || 0}</span><span class="stat-label">Referrals</span></div>
    ...
    renderProfile() {
        ...
        el.innerHTML = `
            ...
            <div id="exchangeTab" class="profile-content active"><div class="exchange-card"><div class="balance-row"><div class="balance-pill"><img src="${this.icons.egg}"><span>${this.miningBalance.toFixed(2)} Eggs</span></div><div class="balance-pill"><img src="${this.icons.ton}"><span>${this.availableBalance.toFixed(4)} TON</span></div></div><div class="rate-badge">Rate: ${APP_CONFIG.EXCHANGE_RATE} Eggs = 1 TON</div><div class="exchange-group"><input type="number" id="exAmount" class="form-input" placeholder="Eggs amount (multiple of 100)"><button id="exBtn" class="submit-btn">Exchange</button></div></div></div>
