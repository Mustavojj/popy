...
class App {
    ...
    constructor() {
        ...
        this.pages = [
            { id: 'tasks-page', name: 'Earn', icon: 'fa-coins', color: '#2ecc71' },
            { id: 'referrals-page', name: 'Invite', icon: 'fa-user-plus', color: '#2ecc71' },
            { id: 'profile-page', name: 'Profile', icon: 'user-photo', color: '#2ecc71' }
        ];
    ...
    updateLoadingStep(step, text, icon = 'fa-spinner fa-pulse', success = false) {
        ...
        const iconColor = success ? '#4CAF50' : (icon.includes('fa-pulse') ? '#2ecc71' : '#f44336');
        ...
        stepData.element.style.color = success ? '#4CAF50' : (icon.includes('fa-pulse') ? '#2ecc71' : '#f44336');
        stepData.element.style.borderLeftColor = success ? '#4CAF50' : (icon.includes('fa-pulse') ? '#2ecc71' : '#f44336');
    ...
    showMaintenancePage() {
        document.body.innerHTML = `
            <div class="maintenance-container">
                <div class="maintenance-content">
                    ...
                    <a href="${APP_CONFIG.NEWS_CHANNEL_LINK}" target="_blank" class="news-channel-btn"><i class="fab fa-telegram"></i> Follow News Channel</a>
                </div>
            </div>
        `;
        ...
        style.textContent = `
            ...
            .maintenance-icon { font-size: 64px; color: #2ecc71; margin-bottom: 20px; }
            .maintenance-content h2 { color: #2ecc71; margin-bottom: 15px; font-size: 24px; }
            .maintenance-content p { color: #e0e0e0; margin: 10px 0; line-height: 1.5; }
            .news-channel-btn {
            ...
            .news-channel-btn:hover { transform: translateY(-2px); }
        `;
    ...
    showBannedPage() {
        document.body.innerHTML = `
            <div class="banned-container" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #0a1a0f; display: flex; align-items: center; justify-content: center; z-index: 10000;">
                <div class="banned-card" style="background: linear-gradient(135deg, #1e3223 0%, #0f1a12 100%); border-radius: 24px; padding: 32px 24px; text-align: center; border: 1px solid rgba(46, 204, 113, 0.3); box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); max-width: 320px; margin: 20px;">
                    <div class="banned-icon" style="font-size: 64px; color: #f44336; margin-bottom: 20px;"><i class="fas fa-ban"></i></div>
                    <h2 style="color: #f44336; font-size: 24px; margin-bottom: 12px;">Access Denied</h2>
                    <p style="color: #d5f5e3; font-size: 14px; line-height: 1.5; margin-bottom: 24px;">This account has been blocked for security reasons. This block is permanent and cannot be reversed.</p>
                    <button onclick="if(window.Telegram?.WebApp) window.Telegram.WebApp.close()" class="close-app-btn" style="background: linear-gradient(135deg, #f44336, #d32f2f); border: none; border-radius: 50px; padding: 12px 24px; color: white; font-weight: bold; font-size: 16px; cursor: pointer; transition: transform 0.3s; display: inline-flex; align-items: center; gap: 8px;"><i class="fas fa-times-circle"></i> Close App</button>
                </div>
            </div>
        `;
    }
        ...
        document.body.innerHTML = `
            <div class="banned-container" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #0a1a0f; display: flex; align-items: center; justify-content: center; z-index: 10000;">
                <div class="banned-card" style="background: linear-gradient(135deg, #1e3223 0%, #0f1a12 100%); border-radius: 24px; padding: 32px 24px; text-align: center; border: 1px solid rgba(46, 204, 113, 0.3); box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); max-width: 320px; margin: 20px;">
                    ...
                    <p style="color: #d5f5e3; font-size: 14px; line-height: 1.5; margin-bottom: 24px;">This device is already registered with another account. Multiple accounts per device are not allowed.</p>
                    <button onclick="if(window.Telegram?.WebApp) window.Telegram.WebApp.close()" class="close-app-btn" style="background: linear-gradient(135deg, #f44336, #d32f2f); border: none; border-radius: 50px; padding: 12px 24px; color: white; font-weight: bold; font-size: 16px; cursor: pointer; transition: transform 0.3s; display: inline-flex; align-items: center; gap: 8px;"><i class="fas fa-times-circle"></i> Close App</button>
                </div>
            </div>
        `;
    ...
    showLaunchButton() {
        ...
        if (steps) steps.style.opacity = '0.9';
        ...
        launchBtn.onclick = () => {
            ...
            if (app) {
                ...
                setTimeout(() => { app.style.opacity = '1'; app.style.transition = 'opacity 0.3s ease'; }, 50);
            }
            this.initializeInAppAds();
            this.showPage('tasks-page');
        };
        const container = loader.querySelector('.loading-container');
        if (container) container.appendChild(launchBtn);
    }
    ...
    async loadUserCreatedTasks() {
        const result = await this.apiCall('getUserCreatedTasks');
        if (result && result.tasks) {
            this.userCreatedTasks = result.tasks;
        } else {
            this.userCreatedTasks = [];
        }
    }
    ...
    async loadAdditionalRewards() {
        const result = await this.apiCall('getAdditionalRewards');
        if (result && result.rewards) {
            this.additionalRewards = result.rewards;
        } else {
            this.additionalRewards = [];
        }
    }
    ...
    async showAddTaskModal() {
        ...
        modal.innerHTML = `
            <div class="task-modal-content">
                <button class="task-modal-close" id="task-modal-close"><i class="fas fa-times"></i></button>
                ...
                <div id="add-task-tab" class="task-modal-body" style="display: block;">
                    <form class="add-task-form" id="add-task-form">
                        <div class="form-group">
                            <label class="form-label"><i class="fas fa-tag"></i> Task Name</label>
                        ...
                        <div class="form-group">
                            <label class="form-label"><i class="fas fa-link"></i> Task Link</label>
                            <input type="url" id="task-link" class="form-input" placeholder="https://t.me/..." required>
                        </div>
                        <div class="form-group">
                            <label class="form-label"><i class="fas fa-shield-alt"></i> Verification Required</label>
                        ...
                        <div id="upgrade-admin-container" style="display: none;">
                            <button type="button" class="upgrade-admin-btn" id="upgrade-admin-btn"><i class="fab fa-telegram"></i> Add @${this.appConfig.BOT_USERNAME} as admin</button>
                        </div>
                        <div class="form-group">
                            <label class="form-label"><i class="fas fa-chart-line"></i> Completions</label>
                            <div class="completions-selector">
                                ${completionsOptions.map(opt => {
                                    ...
                                    return `<div class="completion-option ${opt === 100 ? 'active' : ''}" data-completions="${opt}" data-price="${price}">${opt}</div>`;
                                }).join('')}
                            </div>
                        </div>
                        ...
                        <button type="button" class="pay-task-btn" id="pay-task-btn"><i class="fas fa-coins"></i> Pay ${APP_CONFIG.TASK_PRICE_PER_100_COMPLETIONS} POP</button>
                    </form>
                </div>
                <div id="mytasks-tab" class="task-modal-body" style="display: none;">
                    <div class="my-tasks-list" id="my-tasks-list">${this.renderMyTasks()}</div>
                </div>
            </div>
        `;
    ...
    renderMyTasks() {
        if (!this.userCreatedTasks || this.userCreatedTasks.length === 0) {
            return `<div class="no-data"><i class="fas fa-tasks"></i><p>No tasks created yet</p><p class="hint">Create your first task to earn POP!</p></div>`;
        }
            ...
            return `
                <div class="my-task-item" data-task-id="${task.id}">
                    <div class="my-task-header">
                        <div class="my-task-avatar"><img src="${this.appConfig.BOT_AVATAR}" alt="Task"></div>
                    ...
                    <div class="my-task-progress">
                        <div class="progress-header"><span>Progress</span><span>${currentCompletions}/${maxCompletions}</span></div>
    ...
    setupTaskModalEvents(modal, completionsOptions) {
        ...
        completionOptions.forEach(opt => {
            opt.addEventListener('click', () => {
                ...
                if (userPOP < price) payBtn.disabled = true;
                else payBtn.disabled = false;
            });
        });
        payBtn.addEventListener('click', async () => { await this.handleCreateTask(modal); });
        ...
        try {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (myTasksList) myTasksList.innerHTML = this.renderMyTasks();
    ...
    async checkBotAdminStatus(chatId) {
        const result = await this.apiCall('check-bot-admin', { chatId });
    ...
    async loadUserData(forceRefresh = false) {
        ...
        try {
            const result = await this.apiCall('getUser', { userId: this.tgUser.id });
    ...
    async loadHistoryData() {
        const result = await this.apiCall('getHistory', { userId: this.tgUser.id });
        if (result && !result.error) {
            this.userWithdrawals = result;
        } else {
            this.userWithdrawals = [];
        }
    }
    ...
    startReferralMonitor() {
        if (this.referralMonitorInterval) clearInterval(this.referralMonitorInterval);
        this.referralMonitorInterval = setInterval(async () => {
            if (this.referralManager) await this.referralManager.checkReferralsVerification();
            if (this.tgUser && this.tgUser.id) await this.processPendingReferralsForReferrer(this.tgUser.id);
        }, 30000);
    }

    async processPendingReferralsForReferrer(referrerId) {
        if (this.referralManager) await this.referralManager.processPendingReferrals(referrerId);
    }
        ...
        document.body.innerHTML = `
            <div class="error-container">
                <div class="error-content">
                    <div class="error-header"><div class="error-icon"><i class="fab fa-telegram"></i></div><h2>POP BUZZ</h2></div>
                    <div class="error-message"><div class="error-icon-wrapper"><i class="fas fa-exclamation-triangle"></i></div><h3>Error</h3><p>${message}</p></div>
                    <button onclick="window.location.reload()" class="reload-btn"><i class="fas fa-redo"></i> Reload App</button>
                </div>
            </div>
        `;
    }
    ...
    updateHeader() {
        ...
        if (userPhoto) {
            ...
            userPhoto.style.border = `1px solid #2ecc71`;
            userPhoto.style.boxShadow = '0 4px 15px rgba(46, 204, 113, 0.3)';
        ...
        if (headerBalance) {
            ...
            if (existingBalanceCards) existingBalanceCards.remove();
            ...
            balanceCards.innerHTML = `
                <div class="balance-card"><img src="https://cdn-icons-png.flaticon.com/512/12114/12114247.png" class="balance-icon" alt="TON"><span class="balance-ton">${tonBalance.toFixed(3)}</span></div>
                <div class="balance-card"><img src="https://cdn-icons-png.flaticon.com/512/8074/8074685.png" class="balance-icon" alt="POP"><span class="balance-pop">${Math.floor(popBalance)}</span></div>
            `;
            headerBalance.appendChild(balanceCards);
        }
        const bottomNavPhoto = document.getElementById('bottom-nav-user-photo');
        if (bottomNavPhoto && this.tgUser.photo_url) bottomNavPhoto.src = this.tgUser.photo_url;
    }
    ...
    renderUI() {
        ...
        document.body.addEventListener('copy', (e) => { e.preventDefault(); return false; });
        document.body.addEventListener('contextmenu', (e) => { if (e.target.tagName === 'IMG') { e.preventDefault(); return false; } });
    }
    ...
    showPage(pageId) {
        ...
        if (targetPage) {
            ...
            if (pageId === 'tasks-page') this.renderTasksPage();
            else if (pageId === 'referrals-page') this.renderReferralsPage();
        ...
        tasksPage.innerHTML = `
            <div id="tasks-content">
                <div class="tasks-tabs">
                    <button class="tab-btn active" data-tab="tasks-tab"><i class="fas fa-tasks"></i> Tasks</button>
                    <button class="tab-btn" data-tab="rewards-tab"><i class="fas fa-gift"></i> Rewards</button>
                </div>
                <div id="tasks-tab" class="tasks-tab-content active">
                    <div class="tasks-subtabs">
                        <button class="tab-btn active" data-subtab="main-tasks-sub"><i class="fas fa-star"></i> Main</button>
                        <button class="tab-btn" data-subtab="partner-tasks-sub"><i class="fas fa-handshake"></i> Partner</button>
                        <button class="tab-btn" data-subtab="social-tasks-sub"><i class="fas fa-users"></i> Social</button>
                    </div>
                    <div id="main-tasks-sub" class="task-subtab active"><div class="task-category"><div class="task-category-header"><h3 class="task-category-title"><i class="fas fa-star"></i> Main Tasks</h3></div><div id="main-tasks-list" class="referrals-list"></div></div></div>
                    <div id="partner-tasks-sub" class="task-subtab"><div class="task-category"><div class="task-category-header"><h3 class="task-category-title"><i class="fas fa-handshake"></i> Partner Tasks</h3></div><div id="partner-tasks-list" class="referrals-list"></div></div></div>
                    <div id="social-tasks-sub" class="task-subtab"><div class="task-category"><div class="task-category-header"><h3 class="task-category-title"><i class="fas fa-users"></i> Social Tasks</h3><button class="add-task-btn" id="add-task-btn"><i class="fas fa-plus"></i> Add Task</button></div><div id="social-tasks-list" class="referrals-list"></div></div></div>
                </div>
                <div id="rewards-tab" class="tasks-tab-content">
                    <div class="rewards-grid">
                        <div class="square-card">
                            <div class="card-header"><div class="card-icon"><i class="fas fa-gift"></i></div><h3 class="card-title">Promo Codes</h3></div>
                            <div class="card-divider"></div>
                            <input type="text" id="promo-input" class="promo-input" placeholder="Enter promo code" maxlength="20">
                            <button id="promo-btn" class="promo-btn"><i class="fas fa-gift"></i> APPLY</button>
                        </div>
    ...
    async loadMainTasks() {
        ...
        try {
            ...
            if (mainTasks.length > 0) {
                ...
                mainTasksList.innerHTML = `<div class="no-tasks"><i class="fas fa-star"></i><p>No main tasks available now</p></div>`;
            }
        } catch (error) {
            mainTasksList.innerHTML = `<div class="no-tasks"><i class="fas fa-exclamation-triangle"></i><p>Error loading main tasks</p></div>`;
        }
    }
    ...
    async loadPartnerTasks() {
        ...
        try {
            ...
            if (partnerTasks.length > 0) {
                ...
                partnerTasksList.innerHTML = `<div class="no-tasks"><i class="fas fa-handshake"></i><p>No partner tasks available now</p></div>`;
            }
        } catch (error) {
            partnerTasksList.innerHTML = `<div class="no-tasks"><i class="fas fa-exclamation-triangle"></i><p>Error loading partner tasks</p></div>`;
        }
    }
    ...
    async loadSocialTasks() {
        ...
        try {
            ...
            if (socialTasks.length > 0) {
                ...
                socialTasksList.innerHTML = `<div class="no-tasks"><i class="fas fa-users"></i><p>No social tasks available now</p></div>`;
            }
        } catch (error) {
            socialTasksList.innerHTML = `<div class="no-tasks"><i class="fas fa-exclamation-triangle"></i><p>Error loading social tasks</p></div>`;
        }
    }
    ...
    async loadAdditionalRewardsContent() {
        ...
        if (this.additionalRewards.length > 0) {
            const rewardsHTML = this.additionalRewards.map(reward => `
                <div class="square-card">
                    <div class="card-header"><div class="card-icon"><i class="fas ${reward.icon}"></i></div><h3 class="card-title">${reward.name}</h3></div>
                    <div class="card-divider"></div>
                    ...
                    <button class="reward-btn promo-btn" data-reward-id="${reward.id}" data-reward-action="${reward.action}" data-reward-url="${reward.actionUrl}"><i class="fas fa-arrow-right"></i> Claim</button>
                </div>
            `).join('');
            ...
            btn.addEventListener('click', async () => {
                ...
                if (action === 'url' && actionUrl) {
                    ...
                    if (Object.keys(updates).length > 0) {
                        await this.apiCall('updateUser', { updates });
                    }
    ...
    renderTaskCard(task) {
        ...
        return `
            <div class="referral-row ${isCompleted ? 'task-completed' : ''}" id="task-${task.id}">
                <div class="referral-row-avatar"><img src="${task.picture || defaultIcon}" alt="Task" oncontextmenu="return false;" ondragstart="return false;"></div>
                <div class="referral-row-info">
                    <p class="referral-row-username">${task.name}</p>
                    <p class="task-description">Complete & Earn TON</p>
                    <div class="task-rewards">
                        <span class="reward-badge"><img src="https://cdn-icons-png.flaticon.com/512/12114/12114247.png" class="reward-icon" alt="TON">${task.reward.toFixed(3)}</span>
                        <span class="reward-badge"><img src="https://cdn-icons-png.flaticon.com/512/8074/8074685.png" class="reward-icon" alt="POP">${task.popReward || 1}</span>
                    </div>
                </div>
                <div class="referral-row-status">
                    <button class="task-btn ${buttonClass}" data-task-id="${task.id}" data-task-url="${task.url}" data-task-verification="${task.verification || 'NO'}" data-task-reward="${task.reward}" data-task-pop="${task.popReward || 1}" ${isDisabled ? 'disabled' : ''}>${buttonText}</button>
                </div>
            </div>
        `;
    }
    ...
    setupPromoCodeEvents() {
        ...
        if (promoBtn) promoBtn.addEventListener('click', () => this.handlePromoCode());
        if (promoInput) promoInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') this.handlePromoCode(); });
    }
    ...
    async handlePromoCode() {
        ...
        try {
            const result = await this.apiCall('promoCode', { code });
            if (result.success) {
                this.rateLimiter.addRequest(this.tgUser.id, 'promo_code');
                await this.loadUserData(true);
                this.updateHeader();
                promoInput.value = '';
                this.showNotification("Success", `Promo code applied! +${result.reward.toFixed(3)} TON`, "success");
            } else if (result.channel) {
                this.showJoinRequiredModal(result.channel);
            } else {
                this.showNotification("Promo Code", result.error || "Invalid code", "error");
            }
        } catch (error) {
    ...
    async checkChannelMembership(channelUsername) {
        const result = await this.apiCall('check-channel', { channel: channelUsername, userId: this.tgUser.id });
    ...
    showJoinRequiredModal(channelUsername) {
        ...
        modal.innerHTML = `
            <div class="task-modal-content">
                <button class="task-modal-close" id="modal-close"><i class="fas fa-times"></i></button>
                <div class="task-modal-body">
                    <div class="join-required-content">
                        <div class="join-icon"><i class="fab fa-telegram"></i></div>
                        <h3>Join Required</h3>
                        <p>You need to join the channel to use this promo code:</p>
                        <div class="channel-link"><a href="https://t.me/${channelUsername.replace('@', '')}" target="_blank" class="join-channel-btn"><i class="fab fa-telegram"></i> ${channelUsername}</a></div>
                        <button class="check-join-btn" id="check-join-btn"><i class="fas fa-check-circle"></i> I've Joined</button>
                    </div>
                </div>
            </div>
        `;
    ...
    async completeTask(taskId, url, verification, reward, popReward, button) {
        ...
        try {
            ...
            if (result.success) {
                ...
                this.userState.pop = this.safeNumber(this.userState.pop) + (popReward || 1);
                this.userState.totalTasksCompleted = this.safeNumber(this.userState.totalTasksCompleted) + 1;
                this.userState.completedTasksCount = this.safeNumber(this.userState.completedTasksCount) + 1;
                ...
                this.showNotification("Task Completed!", `+${reward.toFixed(3)} TON, +${popReward || 1} POP`, "success");
            } else {
    ...
    disableAllTaskButtons() {
        document.querySelectorAll('.task-btn:not(.completed):not(.counting):not(:disabled)').forEach(btn => { btn.disabled = true; });
    }

    enableAllTaskButtons() {
        document.querySelectorAll('.task-btn:not(.completed):not(.counting)').forEach(btn => { btn.disabled = false; });
    }
    ...
    async renderReferralsPage() {
        ...
        const referralsList = await this.apiCall('getReferrals', { userId: this.tgUser.id });
        if (referralsList && !referralsList.error) {
            activeReferrals = referralsList.filter(r => r.state === 'verified').length;
        }
        const recentReferrals = referralsList || [];
            ...
            <div class="referrals-container">
                <div class="referral-link-section">
                    <div class="referral-link-box">
                        ...
                        <button class="copy-btn" id="copy-referral-link-btn"><i class="far fa-copy"></i> Copy Link</button>
                    </div>
                    <div class="referral-info">
                        <div class="info-card">
                            <div class="info-icon"><i class="fas fa-gift"></i></div>
                            <div class="info-content"><h4>EARN ${this.appConfig.REFERRAL_BONUS_TON} TON & ${this.appConfig.REFERRAL_BONUS_POP} POP</h4><p>For Every Verified Referral</p></div>
                        </div>
                    </div>
                </div>
                <div class="referral-stats-section">
                    ...
                    <div class="stats-grid-two">
                        <div class="stat-card"><div class="stat-icon"><i class="fas fa-users"></i></div><div class="stat-info"><h4>Total Referrals</h4><p class="stat-value">${totalReferrals}</p></div></div>
                        <div class="stat-card"><div class="stat-icon"><i class="fas fa-user-check"></i></div><div class="stat-info"><h4>Active Referrals</h4><p class="stat-value">${activeReferrals}/${totalReferrals}</p></div></div>
                        <div class="stat-card"><div class="stat-icon"><i class="fas fa-coins"></i></div><div class="stat-info"><h4>TON Earnings</h4><p class="stat-value">${referralEarnings.toFixed(3)} TON</p></div></div>
                        <div class="stat-card"><div class="stat-icon"><i class="fas fa-star"></i></div><div class="stat-info"><h4>POP Earnings</h4><p class="stat-value">${Math.floor(referralPopEarnings)} POP</p></div></div>
                    </div>
                </div>
                <div class="last-referrals-section">
                    ...
                    <div class="referrals-list" id="referrals-list">
                        ${recentReferrals.length > 0 ? recentReferrals.slice(0, 10).map(referral => this.renderReferralRow(referral)).join('') : '<div class="no-data"><i class="fas fa-handshake"></i><p>No referrals yet</p><p class="hint">Share your link to earn!</p></div>'}
                    </div>
                </div>
            </div>
        `;
    ...
    renderReferralRow(referral) {
        ...
        return `
            <div class="referral-row">
                <div class="referral-row-avatar"><img src="${referral.photoUrl}" alt="${referral.firstName}" oncontextmenu="return false;" ondragstart="return false;"></div>
                <div class="referral-row-info"><p class="referral-row-username">${referral.username}</p></div>
                <div class="referral-row-status ${referral.state}">${statusText}</div>
            </div>
        `;
    }
    ...
    async renderProfilePage() {
        ...
        const joinDate = new Date(this.userState.createdAt || Date.now());
        ...
        profilePage.innerHTML = `
            <div class="profile-container">
                <div class="profile-tabs">
                    <button class="profile-tab active" data-profile-tab="deposit-tab"><i class="fas fa-arrow-down"></i> Deposit</button>
                    <button class="profile-tab" data-profile-tab="exchange-tab"><i class="fas fa-exchange-alt"></i> Exchange</button>
                    <button class="profile-tab" data-profile-tab="withdraw-tab"><i class="fas fa-wallet"></i> Withdraw</button>
                </div>
                <div id="deposit-tab" class="profile-tab-content active">
                    <div class="deposit-card">
                        <div class="card-header"><div class="card-icon"><i class="fas fa-arrow-down"></i></div><div class="card-title">Deposit TON</div></div>
                        <div class="card-divider"></div>
                        <div class="deposit-info">
                            <div class="deposit-row"><span class="deposit-label">Wallet:</span><span class="deposit-value" id="deposit-wallet">${this.truncateAddress(this.appConfig.DEPOSIT_WALLET)}</span><button class="deposit-copy-btn" data-copy="wallet"><i class="far fa-copy"></i></button></div>
                            <div class="deposit-row"><span class="deposit-label">Comment:</span><span class="deposit-value" id="deposit-comment">${depositComment}</span><button class="deposit-copy-btn" data-copy="comment"><i class="far fa-copy"></i></button></div>
                            <div class="deposit-actions"><a href="${directPayUrl}" target="_blank" class="direct-pay-btn" id="direct-pay-btn"><i class="fas fa-bolt"></i> Direct Pay</a></div>
                            <div class="deposit-note"><i class="fas fa-info-circle"></i><span>Deposits processed within 1-24 hour</span></div>
                        </div>
                    </div>
                </div>
                <div id="exchange-tab" class="profile-tab-content">
                    <div class="exchange-card">
                        <div class="card-header"><div class="card-icon"><i class="fas fa-exchange-alt"></i></div><div class="card-title">Exchange TON to POP</div></div>
                        <div class="card-divider"></div>
                        <div class="exchange-mini-balance">
                            <div class="mini-balance-item"><img src="https://cdn-icons-png.flaticon.com/512/12114/12114247.png" alt="TON"><span>${this.safeNumber(this.userState.balance).toFixed(3)} TON</span></div>
                            <div class="mini-balance-item"><img src="https://cdn-icons-png.flaticon.com/512/8074/8074685.png" alt="POP"><span>${Math.floor(this.safeNumber(this.userState.pop))} POP</span></div>
                        </div>
                        <div class="exchange-input-group">
                            <div class="amount-input-container">
                                <input type="number" id="exchange-input" class="form-input" placeholder="TON amount" step="0.01" min="${this.appConfig.MIN_EXCHANGE_TON}">
                            ...
                            <button class="exchange-btn" id="exchange-btn"><i class="fas fa-coins"></i> Exchange</button>
                        </div>
                    </div>
                </div>
                <div id="withdraw-tab" class="profile-tab-content">
                    <div class="withdraw-card">
                        <div class="card-header"><div class="card-icon"><i class="fas fa-wallet"></i></div><div class="card-title">Withdraw TON</div></div>
                        <div class="card-divider"></div>
                        <div class="requirements-section">
                            ${!tasksCompleted ? `<div class="requirement-item"><div class="requirement-header"><span><i class="fas fa-tasks"></i> Complete Tasks</span><span class="requirement-count">${tasksProgress}/${tasksRequired}</span></div><div class="progress-bar"><div class="progress-fill" style="width: ${(tasksProgress/tasksRequired)*100}%"></div></div></div>` : ''}
                            ${!referralsCompleted ? `<div class="requirement-item"><div class="requirement-header"><span><i class="fas fa-users"></i> Invite Friends</span><span class="requirement-count">${referralsProgress}/${referralsRequired}</span></div><div class="progress-bar"><div class="progress-fill" style="width: ${(referralsProgress/referralsRequired)*100}%"></div></div></div>` : ''}
                            ${!popCompleted ? `<div class="requirement-item"><div class="requirement-header"><span><i class="fas fa-star"></i> Earn POP</span><span class="requirement-count">${popProgress}/${popRequired}</span></div><div class="progress-bar"><div class="progress-fill" style="width: ${(popProgress/popRequired)*100}%"></div></div></div>` : ''}
                        </div>
                        <div class="form-group"><label class="form-label" for="profile-wallet-input"><i class="fas fa-wallet"></i> TON Wallet Address</label><input type="text" id="profile-wallet-input" class="form-input" placeholder="Enter your TON wallet address (UQ...)" required></div>
                        <div class="form-group amount-group"><label class="form-label" for="profile-amount-input"><i class="fas fa-gem"></i> Withdrawal Amount</label><div class="amount-input-container"><input type="number" id="profile-amount-input" class="form-input" step="0.00001" min="${this.appConfig.MINIMUM_WITHDRAW}" max="${maxBalance}" placeholder="Min: ${this.appConfig.MINIMUM_WITHDRAW.toFixed(3)} TON" required><button type="button" class="max-btn" id="max-btn">MAX</button></div></div>
                        <button id="profile-withdraw-btn" class="withdraw-btn" ${!canWithdraw || maxBalance < this.appConfig.MINIMUM_WITHDRAW ? 'disabled' : ''}><i class="fas fa-paper-plane"></i> ${canWithdraw ? 'WITHDRAW NOW' : this.getWithdrawButtonText(tasksCompleted, referralsCompleted, popCompleted)}</button>
                    </div>
                    <div class="history-section"><div class="history-list" id="withdrawals-list">${this.renderWithdrawalsHistory()}</div></div>
                </div>
            </div>
        `;
    ...
    renderWithdrawalsHistory() {
        if (!this.userWithdrawals || this.userWithdrawals.length === 0) {
            return `<div class="no-data"><i class="fas fa-history"></i><p>No withdrawal history</p><p class="hint">Your withdrawals will appear here</p></div>`;
        }
            ...
            return `
                <div class="history-item-simple">
                    <div class="history-left">
                        <div class="history-amount-row"><img src="https://cdn-icons-png.flaticon.com/512/12114/12114247.png" class="history-ton-icon" alt="TON"><span class="history-amount">${amount.toFixed(3)}</span></div>
                        <div class="history-time-row"><i class="fas fa-clock"></i><span>${this.formatDateTime(withdrawal.timestamp)}</span></div>
                    </div>
                    <div class="history-right"><span class="history-status-badge ${statusClass}">${statusText}</span></div>
                </div>
            `;
        }).join('');
    }
    ...
    getWithdrawButtonText(tasksCompleted, referralsCompleted, popCompleted) {
        if (!tasksCompleted) return `COMPLETE ${this.appConfig.REQUIRED_TASKS_FOR_WITHDRAWAL} TASKS`;
        if (!referralsCompleted) return `INVITE ${this.appConfig.REQUIRED_REFERRALS_FOR_WITHDRAWAL} FRIEND`;
        if (!popCompleted) return `EARN ${this.appConfig.REQUIRED_POP_FOR_WITHDRAWAL} POP`;
        return 'WITHDRAW NOW';
    }
    ...
    setupProfilePageEvents() {
        ...
        copyButtons.forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                let text = '';
                if (type === 'wallet') text = this.appConfig.DEPOSIT_WALLET;
                else if (type === 'comment') text = this.tgUser.id.toString();
                if (text) {
        ...
        if (exchangeBtn) exchangeBtn.addEventListener('click', () => this.exchangeTonToPop());
        ...
        if (exchangeInput && exchangePreview) {
            exchangeInput.addEventListener('input', () => {
                ...
                if (value > 0) exchangePreview.style.opacity = '1';
                else exchangePreview.style.opacity = '0.7';
            });
        }
        if (exchangeMaxBtn && exchangeInput) {
            exchangeMaxBtn.addEventListener('click', () => {
                ...
                if (exchangePreview) exchangePreview.textContent = `≈ ${popAmount} POP`;
            });
        }
        ...
        if (withdrawBtn) {
            withdrawBtn.addEventListener('click', async () => { await this.handleProfileWithdrawal(walletInput, amountInput, withdrawBtn); });
        }
        if (amountInput) {
            amountInput.addEventListener('input', () => {
                const max = this.safeNumber(this.userState.balance);
                const value = parseFloat(amountInput.value) || 0;
                if (value > max) amountInput.value = max.toFixed(3);
            });
        }
    }
    ...
    async exchangeTonToPop() {
        try {
            ...
            try {
                const result = await this.apiCall('exchange', { tonAmount });
                if (result.success) {
                    await this.loadUserData(true);
                    exchangeInput.value = '';
                    const exchangePreview = document.getElementById('exchange-preview');
                    if (exchangePreview) exchangePreview.textContent = '≈ 0 POP';
                    this.updateHeader();
                    this.showNotification("Success", `Exchanged ${tonAmount.toFixed(3)} TON to ${result.popAmount} POP`, "success");
                } else {
    ...
    async handleProfileWithdrawal(walletInput, amountInput, withdrawBtn) {
        ...
        if (totalTasksCompleted < requiredTasks) {
            this.showNotification("Tasks Required", `You need to complete ${requiredTasks - totalTasksCompleted} more tasks to withdraw`, "error");
            return;
        }
        if (totalReferrals < requiredReferrals) {
            this.showNotification("Referrals Required", `You need to invite ${requiredReferrals - totalReferrals} more friend${requiredReferrals - totalReferrals > 1 ? 's' : ''} to withdraw`, "error");
            return;
        }
        if (totalPOP < requiredPOP) {
            this.showNotification("POP Required", `You need to earn ${requiredPOP - totalPOP} more POP to withdraw`, "error");
            return;
        }
        let adShown = false;
        if (typeof window.AdBlock2 !== 'undefined') {
            try { await window.AdBlock2.show(); adShown = true; } catch (error) {}
        }
        if (!adShown) {
        ...
        try {
            const result = await this.apiCall('withdraw', { amount, walletAddress });
            if (result.success) {
                await this.loadUserData(true);
                walletInput.value = '';
                amountInput.value = '';
                ...
                this.showNotification("Error", result.error || "Failed to process withdrawal", "error");
    ...
    setupEventListeners() {
        ...
        if (telegramIdElement) {
            telegramIdElement.addEventListener('click', () => {
                if (this.tgUser?.id) this.copyToClipboard(this.tgUser.id.toString());
            });
        }
    }
    ...
    extractReferralId(startParam) {
        ...
        if (!isNaN(startParam)) return parseInt(startParam);
        ...
        if (startParam.includes('=')) {
            ...
            if (parts.length > 1 && !isNaN(parts[1])) return parseInt(parts[1]);
        }
        return null;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    if (!window.Telegram || !window.Telegram.WebApp) {
        document.body.innerHTML = `
            <div class="error-container">
                <div class="error-content">
                    <div class="error-icon"><i class="fab fa-telegram"></i></div>
                    <h2>POP BUZZ</h2>
